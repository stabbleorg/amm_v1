...
pub fn process_shutdown<'a, 'b, 'c, 'info>(ctx: Context<'_, '_, 'info, 'info, Shutdown<'info>>) -> Result<()> {
    if ctx.remaining_accounts.len() == 2 {
        ...
        let price_update_account = &ctx.remaining_accounts[0];
        let price_feed_account = &ctx.remaining_accounts[1];
        ...
        let index = ctx.accounts.pool.get_token_index(price_feed.mint).unwrap();
        ...
        let weight_f = token.weight as f64 / fixed_math::ONE as f64;

        let total = amount_f * price_f / weight_f;
