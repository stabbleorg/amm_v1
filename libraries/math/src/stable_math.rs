...
pub fn calc_invariant(amplification: U256, balances: &Vec<U256>) -> Result<U256, StableMathError> {
    ...
    for _ in 0..255 {
        ...
        invariant = amp_times_total
            .checked_mul_div_down(sum, get_amp_precision())
            .unwrap()
...
pub fn calc_pool_token_out_given_exact_tokens_in(
    ...
    for i in 0..balances.len() {
        let current_weight = balances[i]
            .checked_mul_div_down(get_inv_precision(), sum_balances)
            .unwrap();
        balance_ratios_with_fee.push(
            balances[i]
                .checked_add(amounts_in[i])
                .unwrap()
                .checked_mul_div_down(get_inv_precision(), balances[i])
                .unwrap(),
        );
        invariant_ratio_with_fees = balance_ratios_with_fee[i]
            .checked_mul_div_down(current_weight, get_inv_precision())
            .unwrap()
    ...
    let invariant_ratio = new_invariant
        .checked_mul_div_down(get_inv_precision(), current_invariant)
        .unwrap();
    ...
    if invariant_ratio > get_inv_precision() {
        let amount_out = pool_token_supply
            .checked_mul_div_down(invariant_ratio.saturating_sub(get_inv_precision()), get_inv_precision())
            .unwrap();
        Ok(amount_out)
    } else {
        Ok(U256::zero())
...
pub fn calc_token_out_given_exact_pool_token_in(
    ...
    let taxable_amount = amount_out_without_fee
        .checked_mul_div_up(taxable_percentage, get_inv_precision())
        .unwrap();
...
fn get_token_balance_given_invariant_n_all_other_balances(
    ...
    let c = invariant_2
        .checked_mul_div_up(get_amp_precision(), amp_times_total.checked_mul(p).unwrap())
        .unwrap()
        .checked_mul(balances[token_index])
        .unwrap();
    let b = invariant
        .checked_mul_div_up(get_amp_precision(), amp_times_total)
        .unwrap()
        .checked_add(sum)
        .unwrap();
...
mod tests {
    ...
    fn test_calc_out_given_in() {
        ...
        let balances = vec![
            uint256!(40_000_000_000_000_000_u64),
            uint256!(60_000_000_000_000_000_u64),
        ];
        let invariant = calc_invariant(amplification, &balances).unwrap();

        println!("Invariant: {}", invariant.as_u64());

        let token_amount_in = uint256!(100_000_000_000_000_u64);
        ...
        println!("A-B out: {}", token_a_out.as_u64());
        println!("B-A out: {}", token_b_out.as_u64());
